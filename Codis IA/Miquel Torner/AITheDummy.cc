...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   map<int,int> rondes_predefinit;
   map<int,Pos> moviment_predefinit;
   ...
   int grup_maxim_ciutat = 6;       // Número màxim d'unitats que atacaran a una ciutat
   int grup_minim_ciutat = 1;
   int grup_maxim_cami = 5;         // Número màxim d'unitats que atacaran a un camí
  ...
  /////////////////////////////// OBTAIN DATA //////////////////////////////////
     ...
     if (round() == 0)
     {
       ...
       if (directtion_center)
       {
         ...
         for (int i : unitats)
         {
           if (random(0,10)%2 == 0)
           {
             sub_find_closer_centered(unit(i), pos_objectiu, false);
           }
         }
         // for (pair<int,Pos> p : moviment_predefinit)
         // {
         //   cerr << p.first << " moves directioned to " << p.second.i << " " << p.second.j << endl;
         // }
       }
     }
     obtain_content_data();
   ...
   void random_queue_shuffle(queue< pair <pair<int, int>, Dir >>& q )
   {
     pair<pair<int, int>, Dir> temp;
     int i = random(0,5);
     while (i--)
     {
       temp = q.front();
   ...
   //////////////////////// FIND CLOSER CENTERED ///////////////////////////////////

   // Calls the find closer funcition, this sub function allow us to know the first step we should do
   Dir sub_find_closer_centered(const Unit& unitat, Pos& pos_objectiu, bool to_objective)
   {
     ...
     if (to_objective)
     {
       ...
       x = unitat.pos.i;
       y = unitat.pos.j;
       if (x != 0 and mapa[x-1][y] != WALL and not visitat[x-1][y])                      q.push(make_pair(make_pair(x-1, y), TOP));
       if (x != (int)mapa.size()-1 and mapa[x+1][y] != WALL and not visitat[x+1][y])     q.push(make_pair(make_pair(x+1, y), BOTTOM));
       if (y != 0 and mapa[x][y-1] != WALL and not visitat[x][y-1])                      q.push(make_pair(make_pair(x, y-1), LEFT));
       if (y != (int)mapa[0].size()-1 and mapa[x][y+1] != WALL and not visitat[x][y+1])  q.push(make_pair(make_pair(x, y+1), RIGHT));
       visitat[x][y] = true;
       random_queue_shuffle(q);
       r = find_closer_objective(unitat, q, visitat, pos_objectiu);
     }
     else
     {
       x = n_rows/2;
       y = n_cols/2;
       queue <pair <pair<int, int>, Pos >> q;
       if (x != 0 and mapa[x-1][y] != WALL and not visitat[x-1][y])                      q.push(make_pair(make_pair(x-1, y), Pos(-1,-1)));
       if (x != (int)mapa.size()-1 and mapa[x+1][y] != WALL and not visitat[x+1][y])     q.push(make_pair(make_pair(x+1, y), Pos(-1,-1)));
       if (y != 0 and mapa[x][y-1] != WALL and not visitat[x][y-1])                      q.push(make_pair(make_pair(x, y-1), Pos(-1,-1)));
       if (y != (int)mapa[0].size()-1 and mapa[x][y+1] != WALL and not visitat[x][y+1])  q.push(make_pair(make_pair(x, y+1), Pos(-1,-1)));
       visitat[x][y] = true;
       pair<Pos,Pos> r2 = find_closer_centered(unitat, q, visitat);
       pos_objectiu = r2.first;
       //moviment_predefinit[unitat.id] = Pos(50, 50);
       //cerr << "moviment predefinit " << r2.second.i << " " << r2.second.j << endl;
       moviment_predefinit[unitat.id] = r2.second;
       //rondes_predefinit[unitat.id] = 100;
       rondes_predefinit[unitat.id] = (abs(unitat.pos.i-r2.second.i) + abs(unitat.pos.j-r2.second.j)) - 3;
     }
   ...
   pair<Pos,Dir> find_closer_objective(const Unit& unitat, queue <pair <pair<int, int>, Dir >>& q, VVB& visitat, Pos pos_objectiu)
   {
      Cell temp_cell;
      int x, y;
      while (!q.empty()) {
        ...
        if (x == pos_objectiu.i and y == pos_objectiu.j)
        {
          return make_pair(Pos(x,y), temp_dir);
        }
        if (!visitat[x][y]) {
          visitat[x][y] = true;
   ...
   pair<Pos,Pos> find_closer_centered(const Unit& unitat, queue <pair <pair<int, int>, Pos >>& q, VVB& visitat)
   {
      Pos no_pos = Pos(-1,-1);
      Cell temp_cell;
      int x, y;
      while (!q.empty()) {
        x = q.front().first.first;
        y = q.front().first.second;
        Pos temp_pos = q.front().second;
        q.pop();

        if (is_city(x,y) and temp_pos == no_pos)
        {
          //cerr << "Found city for unit " << unitat.id << " on " << x << " "  << y << endl;
          temp_pos = Pos(x,y);
        }

        if (x == unitat.pos.i and y == unitat.pos.j)
        {
          //cerr << "Returning temp_pos " << temp_pos << endl;
          return make_pair(Pos(x,y), temp_pos);
        }
        if (!visitat[x][y]) {
          visitat[x][y] = true;
          if (x != 0 and mapa[x-1][y] != WALL)                     q.push(make_pair(make_pair(x-1, y), temp_pos));
          if (x != (int)mapa.size()-1 and mapa[x+1][y] != WALL)    q.push(make_pair(make_pair(x+1, y), temp_pos));
          if (y != 0 and mapa[x][y-1] != WALL)                     q.push(make_pair(make_pair(x, y-1), temp_pos));
          if (y != (int)mapa[0].size()-1 and mapa[x][y+1] != WALL) q.push(make_pair(make_pair(x, y+1), temp_pos));
        }
      }
      return make_pair(Pos(-1,-1), no_pos);
   }
   ...
   void calcular_premoviments()
   {
     ...
     for (int i = 0; i < (int)unitats.size(); ++i)
     {
       ...
       if (directtion_center and rondes_predefinit.find(unitat.id) != rondes_predefinit.end())
       {
         if (rondes_predefinit[unitat.id] > 0)
         {
           rondes_predefinit[unitat.id]--;
           //if (moviment_predefinit[unita])
           //cerr << "GOING TO PREDEFINIT " << moviment_predefinit[unitat.id].i << " " << moviment_predefinit[unitat.id].j << endl;
           next_step = sub_find_closer_centered(unitat, moviment_predefinit[unitat.id], true);
         }
         else rondes_predefinit.erase(unitat.id);
       }
