...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   bool kamikaze = true;
   int distancia_max_kamikaze = 20;  // Distància màxima que un kamikaze atacarà
   int rounds_kamikaze = 100;        // Rondes fins les que els kamikazes estaràn actius, després es comportaran com unitats normals
   ...
   int unitats_lliures = 9;         // Mai campejaran
   ...
   {
     return sqrt(pow(abs(x-pos_u.i),2) + pow(abs(y-pos_u.j), 2)) < distancia;
   }
   ...
   void calcular_premoviments()
   {
     ...
     for (int i = 0; i < (int)unitats.size(); ++i)
     {
       ...
       if (next_step == NONE)
       {
         ...
         if (next_step != NONE and vida_enemic(unitat.pos, next_step) > 70 and unitat.health < 30)
         {
